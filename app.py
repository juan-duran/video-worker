...
def mux_upload(payload: dict = Body(...), x_token: str = Header(default="")):
    ...
    # ---- normalize required inputs (STRICT: only thread_id; never rewrite it) ----
    # Capture thread_id ONCE and never mutate it afterwards.
    raw_tid = payload.get("thread_id")
    # If thread_id key is present but non-string (e.g., int), coerce to str without altering value semantics.
    if raw_tid is None:
        return _error_payload("", (payload.get("video_url_clean") or payload.get("video_url") or payload.get("vredd_url") or ""), "missing_thread_id")

    thread_id = str(raw_tid).strip() if isinstance(raw_tid, (str, int)) else str(raw_tid)
    if thread_id == "":
        return _error_payload("", (payload.get("video_url_clean") or payload.get("video_url") or payload.get("vredd_url") or ""), "missing_thread_id")

    # Keep an immutable local reference so nothing below can overwrite it by mistake.
    TID = thread_id

    # Normalize source URL (accept legacy keys but validate strongly)
    source_url = (payload.get("video_url_clean") or payload.get("video_url") or payload.get("vredd_url") or "")
    source_url = (source_url or "").strip()

    if not source_url.startswith("https://v.redd.it/"):
        # preserve original error contract
    ...
    try:
        ...
        try:
            ...
            # Always return a single-item list; thread_id is IDENTICAL to input
        ...
        # never emit a 500; preserve the exact input thread_id (TID)
        return _error_payload(TID if 'TID' in locals() else "", source_url if 'source_url' in locals() else "", f"unexpected_exception: {type(e).__name__}: {e}")
