...
def mux_upload(payload: dict | list = Body(...), x_token: str = Header(default="")):
    ...
    try:
        ...
        if not thread_id:
            # allow legacy name only as an alias if present (still not generated)
            legacy = (payload.get("reddit_id") or "").strip()
            if legacy:
                thread_id = legacy
            else:
        ...
        # Local temp filenames (safe to use a generated id only for local files)
        ...
        # structured error; keep whatever thread_id was sent (or legacy), handle list payload too
        try:
