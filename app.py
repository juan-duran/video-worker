...
def mux_upload(payload: dict = Body(...), x_token: str = Header(default="")):
    ...
    # ---- normalize required inputs (no fallbacks that rewrite thread_id) ----
    source_url = (
    ...
    # Enforce presence of thread_id; never generate or replace it
    if not thread_id:
        # keep behavior consistent with API style: structured error in a list
        return [_error_payload("", source_url, "missing_thread_id")]

    # Keep an immutable copy to guarantee we never change it
    ORIGINAL_THREAD_ID = thread_id

    # Validate source URL
    if not source_url.startswith("https://v.redd.it/"):
        # preserve original message text to avoid breaking upstream checks
        raise HTTPException(status_code=400, detail="vredd_url inválida")

    # Wrap the download+upload flow defensively so nothing ever leaks a 500
    try:
        ...
        if not info or not has_media:
            return [_error_payload(ORIGINAL_THREAD_ID, source_url, "unavailable_or_removed_preflight")]
        ...
        tmp_base      = _short_id(10)  # avoid leaking identifiers in filenames
        ...
        try:
            ...
            return [_error_payload(ORIGINAL_THREAD_ID, source_url, f"yt-dlp_error: {e}")]
        ...
        try:
            ...
            if need_trim and MAX_DURATION_S > 0:
                ...
                try:
                    ...
                    return [_error_payload(ORIGINAL_THREAD_ID, source_url, f"ffmpeg_trim_error: {e}")]
            ...
            if MAX_BYTES > 0:
                try:
                    ...
                    if file_bytes > MAX_BYTES:
                        return [_error_payload(ORIGINAL_THREAD_ID, source_url, f"skipped_large_file: {file_bytes}B > {MAX_BYTES}B")]
                except Exception:
                    pass

            # ---------- unsigned upload to Cloudinary via REST ----------
            up_url = f"https://api.cloudinary.com/v1_1/{CLD_NAME}/video/upload"
            # Use a short random public_id; do NOT expose source or thread identifiers, or the word "reddit"
            ...
            try:
                ...
                return [_error_payload(ORIGINAL_THREAD_ID, source_url, f"cloudinary_exception: {type(e).__name__}: {e}")]
            ...
            if res.status_code >= 300:
                ...
                try:
                    ...
                    pass
                return [_error_payload(ORIGINAL_THREAD_ID, source_url, f"cloudinary_error: {res.status_code} {snippet}")]
            ...
            # Always return a single-item list as requested; thread_id stays identical to input
            return [ _stable_response(ORIGINAL_THREAD_ID, source_url, public_id=public_id, secure_url=secure_url, thumb_url=thumb_url) ]
        ...
        # last-ditch guard: never emit a 500; keep the original thread_id unmodified
        return [_error_payload(ORIGINAL_THREAD_ID, source_url, f"unexpected_exception: {type(e).__name__}: {e}")]
